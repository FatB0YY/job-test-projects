...
export const Todos: FC = () => {
  ...
  const filteredTodos = useMemo(() => {
    return todos.filter((todo) => {
      ...
      let isFilteredIn: boolean = false
      if (filter === FilterValueObj.ALL) {
        isFilteredIn = true // Все задачи
      } else if (filter === FilterValueObj.ACTIVE && isActive) {
        isFilteredIn = true // Только не завершённые
      } else if (filter === FilterValueObj.COMPLETED && isCompleted) {
        isFilteredIn = true // Только завершённые
      }
